...
// syscall for setting an fd to nonblocking mode
...
// takes one request from the read buffer, generates a response,
...
static bool try_one_request(Conn *conn) {
  ...
  if (4 + len > conn->rbuf_size) {
    // not enough data in hte buffer. Will retry in the next iteration
    return false;
  }
  ...
  // remover the request from the buffer.
...
static bool try_fill_buffer(Conn *conn) {
  ...
  // loop until we hit EAGAIN meaning no more data to read
  do {
    size_t cap = sizeof(conn->rbuf) - conn->rbuf_size;
    rv = read(conn->fd, &conn->rbuf[conn->rbuf_size], cap);
  } while (rv < 0 && errno == EINTR); // EINTR means syscall was interrupted
  if (rv < 0 && errno == EAGAIN) {
    return false; // got EAGAIN, stop.
  }
  if (rv < 0) {
    msg("read() error");
    conn->state = STATE_END;
...
// flushes the write buffer until EAGAIN,
...
// the state machine for client connections
